...
void MainMenu::Draw(int *ptr) {
  ...
  Rectangle start_btn = {(this->width / 2.0f - 41.0f),
                         (this->height / 2.0f - 20.0f), (float)100, (float)40};
  Rectangle Tute_btn = {(this->width / 2.0f - 41.0f),
                        (this->height / 2.0f + 40), (float)100, (float)40};
  mousePos = GetMousePosition();
  BeginDrawing();
  ClearBackground(DARKBLUE);  // Set the background to darkblue
  DrawFPS(10, 10);
  DrawRectangle(start_btn.x, start_btn.y, start_btn.width, start_btn.height,
                GRAY);
  ...
  EndDrawing();
  if (CheckCollisionPointRec(mousePos, start_btn) &&
      IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
    StartPressed(ptr);
  } else if (CheckCollisionPointRec(mousePos, Tute_btn) &&
             IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
    TutePressed(ptr);
  }
}
